...
export default class DocfieldPlaceholderEditing extends Plugin {
  ...
  defineConverters() {
    ...
    conversion
      ...
      .elementToElement({
        ...
        model: (viewElement, modelConversionApi) => {
          ...
          return modelConversionApi.writer.createElement(TYPE_VARIABLE, {
            name,
          });
        },
      });
    ...
    conversion
      ...
      .add((dispatcher) =>
        dispatcher.on("attribute:name", this.editViewPlaceholder),
      );
  ...
  editViewPlaceholder(evt, data, conversionApi) {
    if (data.item && data.item.name === TYPE_PLACEHOLDER) {
      ...
      const placeholder = conversionApi.writer.createText(
