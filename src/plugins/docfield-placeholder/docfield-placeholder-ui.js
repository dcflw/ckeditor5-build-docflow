...
import {
  ...
  TYPE_PLACEHOLDER,
  TYPE_VARIABLE,
} from "./docfield-placeholder-editing";
import { COMMAND_PLACEHOLDER } from "./docfield-placeholder-command";
import { COMMAND_VARIABLE } from "./docfield-variable-command";

export default class DocfieldPlaceholderUi extends Plugin {
  static get requires() {
  ...
  showBalloon(element) {
    ...
    if (element) {
      ...
      if (type === TYPE_PLACEHOLDER) {
        props.selectedPlaceholderId =
          element.getCustomProperty(CUSTOM_PROPERTY_ID);
      } else if (type === TYPE_VARIABLE) {
        props.selectedVariable =
          element.getCustomProperty(CUSTOM_PROPERTY_NAME);
      }
    }
