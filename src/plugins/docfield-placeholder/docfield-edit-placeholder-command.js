...
export default class DocfieldEditPlaceholderCommand extends Command {
      ...
      roots._items.forEach((element, index) => {
        ...
        while (!nodeChild.done) {
          ...
          if (nodeObject.name === "placeholder") {
            const newPlaceholder = newPlaceholders.find(
              (placeholder) => placeholder.id === nodeObject._attrs.get("id"),
            );
            if (newPlaceholder) {
